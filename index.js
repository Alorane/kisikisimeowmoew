...
// Сохраняем состояние пользователя между апдейтами
bot.use(session());
bot.use((ctx, next) => {
...
const REPAIRS_PATH = path.join(__dirname, 'repairs.json');
const ORDERS_PATH = path.join(__dirname, 'orders.json');
const SETTINGS_PATH = path.join(__dirname, 'settings.json');

function normalizeRub(numLike) {
  const n = typeof numLike === 'string'
    ? Number(String(numLike).replace(/[^\d.]/g, ''))
    : Number(numLike);

  if (!Number.isFinite(n)) return null;
  if (n === 0) return 0;
  return n < 300 ? Math.round(n * 100) : Math.round(n);
}

function loadRepairs() {
  try {
    const raw = fs.readFileSync(REPAIRS_PATH, 'utf8');
    const data = JSON.parse(raw);

    for (const model of Object.keys(data)) {
      for (const work of Object.keys(data[model])) {
        const item = data[model][work];
        if (!item) continue;

        if (item.price != null) {
          const rub = normalizeRub(item.price);
          if (rub != null) item.price = rub;
        }

        if (!item.desc) {
          item.desc = 'Сервисная работа. Оригинальные или проверенные комплектующие.';
        }
      }
    }
    ...
    console.error('❌ Не удалось прочитать repairs.json:', e.message);
    return {};
  }
}
...
function loadSettings() {
  try {
    ...
    const data = JSON.parse(raw);
    if (!Array.isArray(data.notifyChats)) data.notifyChats = [];
    if (ADMIN_IDS.length) {
      const set = new Set(data.notifyChats.map(String));
      for (const id of ADMIN_IDS) set.add(String(id));
      data.notifyChats = Array.from(set);
    }
    return data;
  } catch {
    return { notifyChats: ADMIN_IDS.map(String) };
  }
}
...
function saveSettings() {
  try {
    ...
    console.error('❌ Не удалось сохранить settings.json:', e.message);
    return false;
  }
}

function getNotifyChatIds() {
  const chats = new Set((SETTINGS.notifyChats || []).map(String));
  for (const id of ADMIN_IDS) chats.add(String(id));
  return Array.from(chats);
}

function addNotifyChat(chatId) {
  const id = String(chatId);
  if (!SETTINGS.notifyChats.includes(id)) {
    SETTINGS.notifyChats.push(id);
    return saveSettings();
  }
  return true;
}

function removeNotifyChat(chatId) {
  const id = String(chatId);
  const before = SETTINGS.notifyChats.length;
  SETTINGS.notifyChats = SETTINGS.notifyChats.filter((item) => item !== id);
  if (SETTINGS.notifyChats.length !== before) {
    return saveSettings();
  }
  return true;
}
...
function saveRepairs() {
  try {
    fs.writeFileSync(REPAIRS_PATH, JSON.stringify(REPAIRS, null, 2), 'utf8');
    return true;
  } catch (e) {
    console.error('❌ Не удалось сохранить repairs.json:', e.message);
    return false;
  }
}
...
const models = () => Object.keys(REPAIRS).sort();

const descEnhancers = [
  {
    match: /экран/i,
    extra: 'Каждый дисплей проходит проверку на точность цветов и яркость, тачскрин тестируем в 20 точках.'
  },
  {
    match: /аккумулятор/i,
    extra: 'Перед выдачей делаем цикл быстрой и медленной зарядки, чтобы убедиться в стабильности питания.'
  },
  {
    match: /корпус/i,
    extra: 'Переносим все кнопки и антенны аккуратно, чтобы сохранить заводскую геометрию и прочность.'
  },
  {
    match: /камера/i,
    extra: 'Используем антистатическую зону, чтобы не допустить пыли и засветов на новом модуле.'
  },
  {
    match: /порта зарядки|заряд/i,
    extra: 'Также чистим плату от окислов и меняем мелкие элементы, если они влияют на стабильность зарядки.'
  },
  {
    match: /стекла/i,
    extra: 'Стекло клеим в прессе под вакуумом, чтобы не было пыли и равномерно лег поляризатор.'
  }
];

const buildDescription = (issue, base = '') => {
  const extras = descEnhancers
    .filter(({ match }) => match.test(issue))
    .map(({ extra }) => extra);
  if (!extras.length) return base || 'Сервисная работа. Оригинальные или проверенные комплектующие.';
  const unique = Array.from(new Set(extras));
  return `${base || 'Сервисная работа. Оригинальные или проверенные комплектующие.'}\n\n${unique.join('\n')}`;
};
...
const adminModeSet = new Set();
const adminKey = (ctx) => {
  const id = ctx.from?.id;
  return id != null ? String(id) : null;
};
const isAdminMode = (ctx) => {
...
function modelsKeyboard(page = 0) {
  ...
  const rows = chunk(slice.map(m => Markup.button.callback(m, `mdl:${p}:${m}`)), 2);
  const nav = [];
  if (pages > 1) {
    nav.push(
  ...
  const keyboardRows = rows.length ? [...rows] : [[Markup.button.callback('↻ Обновить', `nav:${p}`)]];
  keyboardRows.push(nav.length ? nav : [Markup.button.callback('↻ Обновить', `nav:${p}`)]);
  return Markup.inlineKeyboard(keyboardRows);
}

function issuesKeyboard(model, admin = false) {
  const entries = Object.keys(REPAIRS[model] || {});
  const rows = chunk(entries.map((issue, i) => Markup.button.callback(issue, `iss:${i}`)), 1);
  if (admin) {
    rows.push([Markup.button.callback('➕ Добавить работу', `add_issue:${model}`)]);
  }
...
function buildIssueResponse(model, issue, admin = false) {
...
bot.start(async (ctx) => {
  ...
  ctx.session = {};
  await ctx.reply('Привет! 👋 Я помогу рассчитать ремонт. Сначала выбери модель iPhone:', modelsKeyboard(0));
});
...
bot.action(/^iss:(\d+)$/, async (ctx) => {
  ...
  if (!model || !issues || !issues[idx]) {
    return ctx.answerCbQuery('Сессия сброшена, выбери модель заново.', { show_alert: true });
  }
  const issue = issues[idx];
  const payload = buildIssueResponse(model, issue, isAdminMode(ctx));
  if (!payload) {
    return ctx.answerCbQuery('Не удалось загрузить работу', { show_alert: true });
  }
...
bot.action(/^add_issue:(.+)$/s, async (ctx) => {
  ...
  await ctx.answerCbQuery();
  await ctx.reply(`Добавляем новую работу для ${model}. Напиши название работы:`);
});
...
bot.on('text', async (ctx, next) => {
  ...
  if (isAdminMode(ctx) && ctx.session?.adminEdit) {
    ...
    if (mode === 'price' && issue) {
      ...
      if (!Number.isFinite(num) || num <= 0) {
        return ctx.reply('Не получилось распознать число. Введи цену в рублях, например 12500.');
      }
      REPAIRS[model][issue].price = Math.round(num);
      const saved = saveRepairs();
      if (saved) {
        ctx.reply(`Цена для «${issue}» обновлена: ${fmtPrice(num)}.`);
      } else {
    ...
    if (mode === 'add_issue') {
      if (stage === 'title') {
        if (!text) return ctx.reply('Название не может быть пустым. Попробуй ещё раз.');
        if (!REPAIRS[model]) REPAIRS[model] = {};
  ...
  if (step === 'name') {
    ...
    return ctx.reply('Укажи номер телефона: 10 цифр без 8, например 9000000000.');
  }
  ...
  if (step === 'phone') {
    ...
    if (!normalized.ok) {
      if (normalized.reason === 'startsWith8') {
        return ctx.reply('Вводи номер без 8 в начале. Просто 10 цифр, например 9000000000.');
      }
      return ctx.reply('Номер не похож на телефон. Нужны 10 цифр, например 9000000000.');
    }
    ...
    try {
      const before = fs.existsSync(ORDERS_PATH)
        ? JSON.parse(fs.readFileSync(ORDERS_PATH, 'utf8'))
        : [];
      before.push(order);
      fs.writeFileSync(ORDERS_PATH, JSON.stringify(before, null, 2), 'utf8');
    } catch (e) {
      console.error('Ошибка записи orders.json:', e.message);
    }
    ...
    const notifyTargets = getNotifyChatIds();
  ...
  if (!step) {
    return ctx.reply('Выбери модель из меню ниже или введи её текстом точно как в прайсе:', modelsKeyboard(0));
  }
...
bot.command('admin', async (ctx) => {
  ...
  if (model && REPAIRS[model]) {
    await ctx.reply(`📱 Модель: ${model}\nВыбери неисправность:`, issuesKeyboard(model, true));
    return;
  }
  await ctx.reply('Выбери модель:', modelsKeyboard(0));
});
...
bot.command('reload', async (ctx) => {
  if (!isAdmin(ctx)) {
    if (isPrivateChat(ctx)) await ctx.reply('Команда доступна только администратору.');
    return;
  }
  if (!isPrivateChat(ctx)) {
  ...
  REPAIRS = loadRepairs();
  await ctx.reply('🔄 Прайс перезагружен из repairs.json');
});
...
bot.command('stop_notify', async (ctx) => {
  ...
  const ok = removeNotifyChat(chatId);
  if (ok) {
    await ctx.reply('🔕 Этот чат больше не получает уведомления. Если нужно снова — /notify_here.');
  } else {
...
bot.command('notify_list', async (ctx) => {
  ...
  const chats = getNotifyChatIds();
  if (!chats.length) {
    return ctx.reply('Уведомления никуда не отправляются. Используй /notify_here в нужном чате.');
  }
